...
function loadNotifications() {
  fetch('/api/teacher/notifications2')
...
function updateNotificationUI(notifications) {
  // Sort: seenbyens === NULL first, then by DateTime descending
  notifications.sort((a, b) => {
    if (a.seenbyens === null && b.seenbyens !== null) return -1;
    if (a.seenbyens !== null && b.seenbyens === null) return 1;
  ...
  if (showOnlyUnseen) {
    filteredNotifications = notifications.filter(notif => notif.seenbyens === null);
  }

  const unseenCount = notifications.filter(notif => notif.seenbyens !== 1).length;
...
filteredNotifications.forEach(notif => {
  const isUnread = notif.seenbyens !== 1;
  ...
  const s = notif.status;
    ...
    notificationItem.addEventListener('click', function(e) {
      fetch(`/api/teacher/mark-seenbyens/${notif.id}`, { method: 'POST' })
        .then(() => {
          notif.seenbyens = 1;
          updateNotificationUI(notifications);
          window.location.href = '/NotificationEns';
        });
    });
...
function markAllNotificationsAsRead() {
  fetch('/api/teacher/mark-seenbyens/all', { method: 'POST' })
    .then(() => {
